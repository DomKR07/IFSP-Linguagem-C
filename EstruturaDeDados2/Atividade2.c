...
int main() {
    ...
    int vetOrdenado[] = {100, 110, 120, 130, 140, 150, 160, 170, 180, 190};
    int elem1 = 100;
    int elem2 = 170;
    int elem3 = 130;
    ...
    printf("Insira um novo valor na lista ordenada: ");
    scanf("%d", &novoValor);

    inserirOrdenado(vetOrdenado, &n, novoValor);
    printf("\nNovo Vetor Ordenado: ");
    for (int i = 0; i < n; i++) {
        printf("%d ", vetOrdenado[i]);
    }
...
void inserirOrdenado(int *vetOrdenado, int *n, int novoValor) {
        int i, j;

        for (i = 0; i < *n && novoValor > vetOrdenado[i]; i++);

        for (j = *n; j > i; j--)
            vetOrdenado[j] = vetOrdenado[j - 1];

        vetOrdenado[i] = novoValor;
